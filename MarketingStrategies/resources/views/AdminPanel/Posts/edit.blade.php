                    ...
                    <form runat="server" action="{{route('Post.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex-auto p-4 w-full">
                            <div class="flex flex-row -mx-3 w-full p-3">
                                <input type="text" value="{{$data->url}}" class="pl-2 text-sm focus:shadow-primary-outline ease leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 dark:bg-slate-850 dark:text-white bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none focus:transition-shadow" placeholder="YouTube URLsini kiriting..." required name="url">
                            </div>
                        </div>
                        <div class="flex-auto p-4 w-full">
                            <div class="flex flex-row -mx-3 w-full p-3">
                ...
                <div class="flex flex-wrap items-center -mx-3 lg:justify-between">
                    <div class="w-full max-w-full px-3 mt-0 mb-6 shrink-0 lg:mb-0 lg:w-1/2 lg:flex-none">
                        <div class="text-sm leading-normal text-center text-slate-500 lg:text-left">
                            ...
                            Created by
                            <a href="" class="font-semibold dark:text-white text-slate-700" target="_blank">R.Suhrob</a>.
                        </div>
                    </div>
                </div>
            </div>
...
<!-- main script file  -->
<script src="{{asset('AdminPanel/assets/js/argon-dashboard-tailwind2.js?v=1.0.1')}}" async></script>
...
<script>
    ...
    document.getElementById("imgInp").addEventListener("paste", function(){
        console.log("sdc");
    });
    ...
    imgInp.onchange = evt => {
        ...
        if (file) {
            ...
            if(s.substring(s.length-4,s.length)=='.jpg')
            {
                ...
                document.getElementById('img_window').style="display: block";
            }
        }
    }
</script>
